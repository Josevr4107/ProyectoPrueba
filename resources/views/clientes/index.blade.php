...
<div class="container">
  ...
  <form action="{{ route('buscar_cliente') }}" method="post">
    ...
    <div class="row mt-4">
      ...
      <div class="col-md-3 col-xs-12  mb-3">
        <input placeholder="CI / NIT" type="number" min="1" pattern="^[0-9]+" name="cinit" class="form-control">
      </div>
      ...
      <tr>
        ...
        <td>
          <div class="btn-group" role="group">
            <button id="btn_show" type="button" data-toggle="modal" data-target="#ClienteShow{{ $cliente->id }}"
              class="btn btn-sm btn-secondary"><i class="fas fa-eye"></i></button>

            <button id="btn_edit" type="button" data-toggle="modal" data-target="#ClienteEdit{{ $cliente->id }}"
              class="btn btn-sm btn-secondary"><i class="fas fa-edit"></i></button>

            <button id="btn_estado" type="button" data-toggle="modal" data-target="#ClienteEstado{{ $cliente->id }}"
              class="btn btn-sm btn-secondary"><i class="fas fa-adjust"></i></button>

            <button id="btn_estado" type="button" data-toggle="modal" data-target="#ClienteDestroy{{ $cliente->id }}"
              class="btn btn-danger" {{ $cliente->estado ? 'disabled' : '' }}><i class="fas fa-trash-alt"></i></button>
          </div>
        </td>
      </tr>
