...
@section('content')
    ...
    <div class="container">
        <form action="" class="bg-danger">
            ...
            <select name="cliente"
                class="form-control col-md-5 col-xs-12  mb-3 
            @error('cliente') is-invalid @enderror"
                id="cliente">

                <option value="">-- Seleccione una Cliente -</option>

                @foreach ($clientes as $cliente)
                    <option value="{{ $cliente->id }}" {{-- si la ultima cliente es igual al id actual
                             entonces agregale el atributo selected caso contrario no agrega nada --}}
